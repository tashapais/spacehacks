...
>
	...
	>
		{#if message.role === 'assistant'}
			<div
				class="prose-sm prose-headings:font-bold prose-h1:text-lg prose-h2:text-base prose-h2:mt-4 prose-h2:mb-2 prose-p:my-2 prose-ul:my-2 prose-li:my-1 prose max-w-none"
			>
				{@html renderedText}
			</div>
		{:else}
							...
							<div>
								...
								{#if citation.url}
									<a
										...
										target="_blank"
										href={citation.url}>Open</a
									>
								{/if}
							</div>
							{#if citation.snippet}
								<div class="mt-0.5 line-clamp-3 text-[11px] text-gray-500">
									“{citation.snippet}”
								</div>
							{/if}
						</li>
